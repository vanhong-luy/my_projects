...
<a class="flex items-center gap-3 px-4 py-2.5 rounded-lg sidebar-link active" href="artists">
...
<header class="mb-8">
<h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Artist</h1>
</header>
<section class="mb-8">
<h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
<div class="flex gap-4">
<a href="add_new_artist" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm hover:bg-opacity-90 transition-colors ">
          ...
          <tbody>
            <?php foreach ($artists as $artist): ?>
              <tr class="border-t border-[var(--border-color)] hover:bg-[var(--secondary-color)]">
                <td class="px-4 py-3"><?= htmlspecialchars($artist['id']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($artist['artist_name']) ?></td>
                <td class="px-4 py-3">
                  <!-- Placeholder for edit/delete -->
                  <a href="edit_artist.php?id=<?= $artist['id'] ?>" class="text-blue-600 hover:underline">Edit</a>
                  |
                  <a href="artists.php?action=delete&id=<?= $artist['id']?>" class="text-red-600 hover:underline" onclick="return confirm('Delete this artist?')">Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
            <?php if (empty($artists)): ?>
              <tr><td colspan="3" class="px-4 py-3 text-center text-gray-500">No artists found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
