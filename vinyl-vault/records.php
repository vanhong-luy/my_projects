...
if ($id > 0) {
    $stmt = $pdo->prepare("DELETE FROM record WHERE id = ?");
    $stmt->execute([$id]);
...
<header class="mb-8">
<h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Records</h1>
</header>
<section class="mb-8">
<h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
<div class="flex gap-4">
<a href="add_new_record" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm">
...
<section>
      <div class="overflow-x-auto border border-[var(--border-color)] rounded-lg">
        <table class="min-w-full bg-white text-sm">
          <thead class="bg-[var(--secondary-color)] text-left text-[var(--text-primary)]">
            <tr>
          ...
          <tbody>
            <?php foreach ($records as $record): ?>
              <tr class="border-t border-[var(--border-color)] hover:bg-[var(--secondary-color)]">
                <td class="px-4 py-3"><?= htmlspecialchars($record['id']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($record['record_title']) ?></td>
                <td class="px-4 py-3"><?= number_format($record['price'], 2) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($record['condition']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($record['in_stock']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($record['format_name']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($record['album_title']) ?></td>
                <td class="px-4 py-3">
                  <!-- Placeholder for edit/delete -->
                  <a href="edit_record.php?id=<?= $record['id'] ?>" class="text-blue-600 hover:underline">Edit</a>
                  |
                  <a href="records.php?action=delete&id=<?=$record['id'] ?>" class="text-red-600 hover:underline" onclick="return confirm('Delete this record?')">Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
            <?php if (empty($records)): ?>
              <tr><td colspan="9" class="px-4 py-3 text-center text-gray-500">No records found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
