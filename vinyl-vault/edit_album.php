...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("
        UPDATE album SET
            title = ?, album_duration = ?, release_date = ?, genre_id = ?, artist_id = ?
        WHERE id = ?
    ");
    $stmt->execute([
        $title,
    ...
    // Redirect back to albums page after update
...
// Fetch genres and artists for the select options
...
<div class="max-w-7xl mx-auto">
    <header class="mb-8">
        <h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Edit Album</h1>
    </header>

    <section class="mb-8">
        <h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
        <div class="flex gap-4">
            <a href="albums" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm">
                <span>Return</span>
            </a>
        </div>
    </section>
    ...
    <div class="py-1">
        <form action="edit_album.php?id=<?=$id?>" method="POST" class="space-y-6">
            <!-- CSRF token -->
            ...
            <div>
                <label for="album_duration" class="block mb-1 text-[#9e8747] font-semibold">Duration (minutes)</label>
                <input 
                type="number" 
                id="album_duration" 
                name="album_duration" 
                value="<?= htmlspecialchars($album['album_duration']) ?>"
                class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" 
                required>
            </div>

            <div>
                <label for="release_date" class="block mb-1 text-[#9e8747] font-semibold">Release Date</label>
                <input type="date" id="release_date" name="release_date" value="<?= htmlspecialchars($album['release_date']) ?>" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            </div>

            <div>
                <label for="genre_id" class="block mb-1 text-[#9e8747] font-semibold">Genre</label>
                <select id="genre_id" name="genre_id" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            ...
            <div>
                <label for="artist_id" class="block mb-1 text-[#9e8747] font-semibold">Artist</label>
                <select id="artist_id" name="artist_id" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
