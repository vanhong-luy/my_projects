...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE genre SET genre_name = ? WHERE id = ?");
    $stmt->execute([$genre_name,$id]);

    // Redirect back to genre page after update
    header('Location: genres.php');
    exit;
}

$stmt = $pdo->prepare("SELECT * FROM genre WHERE id = ?");
$stmt->execute([$id]);
$genre = $stmt->fetch();
...
<a class="flex items-center gap-3 px-4 py-2.5 rounded-lg sidebar-link active" href="genres">
...
<header class="mb-8">
<h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Edit Genre</h1>
</header>
<section class="mb-8">
<h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
<div class="flex gap-4">
<a href="genres" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm">
<span>Return</span>
</a>
</div>
</section>
...
<main>
    ...
    <div class="py-1">
        <form action="edit_genre.php?id=<?=$id?>" method="POST" class="space-y-6">
            <!-- CSRF token -->
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">

            <div>
                <label for="genre_name" class="block mb-1 text-[#9e8747] font-semibold">Genre Name</label>
                <input 
                    type="text" 
                    id="genre_id" 
                    name="genre_name" 
                    class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" 
                    value="<?= htmlspecialchars($genre_name) ?>"
                    required>
                </input>
            </div>
