...
<header class="mb-8">
<h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Genre</h1>
</header>
<section class="mb-8">
<h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
<div class="flex gap-4">
<a href="add_new_genre" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm">
        ...
        <table class="min-w-full bg-white text-sm">
          <thead class="bg-[var(--secondary-color)] text-left text-[var(--text-primary)]">
            <tr>
              ...
              <th class="px-4 py-3">Genre Name</th>
          ...
          <tbody>
            <?php foreach ($genres as $genre): ?>
              <tr class="border-t border-[var(--border-color)] hover:bg-[var(--secondary-color)]">
                <td class="px-4 py-3"><?= htmlspecialchars($genre['id']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($genre['genre_name']) ?></td>
                <td class="px-4 py-3">
                  <!-- Placeholder for edit/delete -->
                  <a href="edit_genre.php?id=<?= $genre['id'] ?>" class="text-blue-600 hover:underline">Edit</a>
                  |
                  <a href="genres.php?action=delete&id=<?= $genre['id']?>" class="text-red-600 hover:underline" onclick="return confirm('Delete this Genre?')">Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
            <?php if (empty($genres)): ?>
              <tr><td colspan="3" class="px-4 py-3 text-center text-gray-500">No albums found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
