...
$artists = $pdo->query("SELECT id, artist_name FROM artist ORDER BY artist_name")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO artist (artist_name) VALUES (?)");
...
<div class="max-w-7xl mx-auto">
    <header class="mb-8">
        <h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Add New Artist</h1>
    </header>

    <section class="mb-8">
        <h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
        <div class="flex gap-4">
            <a href="artists" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm hover:bg-opacity-90 transition-colors">
                <span>Return</span>
            </a>
        </div>
    </section>
    ...
    <div class="py-1">
        <form method="POST" class="space-y-6">
            ...
            <div>
                <label for="artist_name" class="block mb-1 text-[#9e8747] font-semibold">Artist Name</label>
                <input type="text" id="artist_name" name="artist_name" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            </div>
