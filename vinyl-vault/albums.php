...
<header class="mb-8">
<h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Albums</h1>
</header>
<section class="mb-8">
<h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
<div class="flex gap-4">
<a href="add_new_album" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm hover:bg-opacity-90 transition-colors ">
        ...
        <table class="min-w-full bg-white text-sm">
          <thead class="bg-[var(--secondary-color)] text-left text-[var(--text-primary)]">
            <tr>
              ...
              <th class="px-4 py-3">Duration (Minutes)</th>
          ...
          <tbody>
            <?php foreach ($albums as $album): ?>
              <tr class="border-t border-[var(--border-color)] hover:bg-[var(--secondary-color)]">
                <td class="px-4 py-3"><?= htmlspecialchars($album['id']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($album['album_title']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($album['duration']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($album['release_date']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($album['genre_name']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($album['artist_name']) ?></td>
                <td class="px-4 py-3">
                  <!-- Placeholder for edit/delete -->
                  <a href="edit_album?id=<?=$album['id'] ?>" class="text-blue-600 hover:underline">Edit</a>
                  |
                  <a href="albums.php?action=delete&id=<?=$album['id'] ?>" class="text-red-600 hover:underline" onclick="return confirm('Delete this album?')">Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
            <?php if (empty($albums)): ?>
              <tr><td colspan="7" class="px-4 py-3 text-center text-gray-500">No albums found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
