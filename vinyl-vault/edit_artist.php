...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE artist SET artist_name = ? WHERE id = ?");
    $stmt->execute([$artist_name,$id]);
...
<header class="mb-8">
<h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Edit Artist</h1>
</header>
<section class="mb-8">
<h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
<div class="flex gap-4">
<a href="artists" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm">
<span>Return</span>
</a>
</div>
</section>
...
<main>
    ...
    <div class="py-1">
        <form action="edit_artist.php?id=<?=$id?>" method="POST" class="space-y-6">
            <!-- CSRF token -->
            ...
            <div>
                ...
                <input 
                    ...
                    value="<?= htmlspecialchars($artist_name) ?>"
                    required>
                </input>
            </div>
