...
// Fetch albums and formats
$albums = $pdo->query("SELECT id, title FROM album ORDER BY title")->fetchAll();
$formats = $pdo->query("SELECT id, format_name FROM format ORDER BY format_name")->fetchAll();
$conditions = ['Mint', 'Near Mint', 'Excellent', 'Very Good', 'Good', 'Fair', 'Poor'];

// Handle form submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $title = $_POST['record_title'] ?? '';
    ...
    // Basic validation could go here (e.g., checking required fields)

    $stmt = $pdo->prepare("
        INSERT INTO record (record_title, price, `condition`, in_stock, album_id, format_id)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$title, $price, $condition, $stock, $album_id, $format_id]);
...
<a class="flex items-center gap-3 px-4 py-2.5 rounded-lg sidebar-link active" href="records">
...
<div class="max-w-7xl mx-auto">
    <header class="mb-8">
        <h1 class="text-4xl font-bold text-[var(--text-primary)] tracking-tight">Add New Records</h1>
    </header>

    <section class="mb-8">
        <h2 class="text-xl font-bold text-[var(--text-primary)] mb-4">Quick Actions</h2>
        <div class="flex gap-4">
            <a href="records" class="flex items-center justify-center h-11 px-6 rounded-md bg-[var(--primary-color)] text-white text-sm font-bold shadow-sm hover:bg-opacity-90 transition-colors">
                <span>Return</span>
            </a>
        </div>
    </section>
    ...
    <div class="py-1">
        <form method="POST" class="space-y-6">
            ...
            <div>
                <label for="record_title" class="block mb-1 text-[#9e8747] font-semibold">Title</label>
                <input 
                type="text" 
                id="record_title" 
                name="record_title" 
                class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" 
                required>
            </div>

            <div>
                <label for="album_id" class="block mb-1 text-[#9e8747] font-semibold">Album</label>
                <select id="album_id" name="album_id" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            ...
            <div>
                <label for="format_id" class="block mb-1 text-[#9e8747] font-semibold">Format</label>
                <select id="format_id" name="format_id" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
                    <option disabled selected value="">-- Select Format --</option>
                    <?php foreach ($formats as $format): ?>
                        <option value="<?= $format['id'] ?>"><?= htmlspecialchars($format['format_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>

            <div>
                <label for="condition" class="block mb-1 text-[#9e8747] font-semibold">Condition</label>
                <select id="condition" name="condition" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            ...
            <div>
                <label for="price" class="block mb-1 text-[#9e8747] font-semibold">Price</label>
                <input type="number" step="0.01" id="price" name="price" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            </div>

            <div>
                <label for="stock" class="block mb-1 text-[#9e8747] font-semibold">Stock</label>
                <input type="number" id="stock" name="stock" class="w-full rounded border border-[#9e8747] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9e8747]" required>
            </div>

            <button type="submit" class="rounded bg-[#9e8747] px-6 py-3 text-white font-semibold hover:bg-[#7a6a36] transition-colors duration-200">
                Save Record
            </button>
        </form>
    </div>
